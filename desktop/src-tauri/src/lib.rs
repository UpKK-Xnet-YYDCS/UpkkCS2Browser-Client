...
#[tauri::command]
async fn open_forum_with_login(
    app: tauri::AppHandle,
    uid: String,
    auth: String,
) -> Result<(), String> {
...
#[tauri::command]
async fn open_url_in_browser_window(
    app: tauri::AppHandle,
    window_label: String,
    url: String,
    title: String,
...
async fn save_monitor_data(app: tauri::AppHandle, data: String) -> Result<(), String> {
    ...
    std::fs::write(&path, data).map_err(|e| format!("Failed to save monitor data: {}", e))
}
...
async fn load_monitor_data(app: tauri::AppHandle) -> Result<String, String> {
    ...
    std::fs::read_to_string(&path).map_err(|e| format!("Failed to read monitor data: {}", e))
}
