...
export function AutoJoinModal({ server, onClose }: AutoJoinModalProps) {
  ...
  // Prefer display_address (domain) over raw IP for Steam connect URLs, matching web behavior
  // 优先使用域名地址构建Steam连接URL，与网页端行为一致
    ...
    try {
      ...
      if (querySuccess) {
        ...
        if (availableSlots >= minSlots) {
          ...
          // Join the server using domain-preferred address
